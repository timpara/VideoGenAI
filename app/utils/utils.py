...
def split_string_by_punctuations(s):
    ...
    for i in range(len(s)):
        ...
        if char == "," and previous_char.isdigit() and next_char.isdigit():
            # The thousands separator inside English numbers is not a
            # sentence break, for example "1,000 years". Edge TTS word
            # boundaries usually return such numbers as a single continuous
            # token; splitting them into "1" and "000 years" here would
            # prevent later subtitle aggregation from matching the original
            # script, which then falls back to Whisper by mistake.
            txt += char
            continue
